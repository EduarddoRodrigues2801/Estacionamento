...
export const activityCheckin = async (request, response) => {
    ...
    response.status(400).send({
        message: 'Veiculo [${label}] não cadastrado' 
    });    
};
    ...
    if (vehicle) {
        ...
        FROM activities
         ...
         if (activityOpen) {
            ...
            db.close();
            response.status(400).send({
                 message: 'Veiculo [${label}] não realizou nenhuma check-in' 
        });   
        return; 
    }
...
export const activityRemove = async (request, response) => {
...
export const activityList = async (request, response) => {
    const db = await openDatabase();
